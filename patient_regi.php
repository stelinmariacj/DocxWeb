	...
	<div class="recipient_reg" style="background-color:#272327;">
		...
		<form enctype="multipart/form-data" method="post" class="text-center">
			 <div class="col-md-12">
			 		...
			 		<label>
						Your Name: <input type="text" name="name" value="" placeholder="Full name" pattern="[a-zA-Z]{1-100}" required>
					</label><br><br>

					<label>
						Age: <input type="number" name="age"  placeholder="age" pattern="[0-9]{2,2}" title="please enter only  numbers between 2 to 2 for age" required>
					</label><br><br>
					<label>
					...
					<label>
						Password: <input type="password" name="password"  value="" placeholder="password"  placeholder="Eg: Aa@123 (6 characters)" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,}"required>
					</label><br><br>
